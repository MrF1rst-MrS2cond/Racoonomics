...
func _ready():
	Global.update_bar_percent.connect(change_progress_bar)

func change_progress_bar(new_percent: float):
	...
	current_tween.tween_property(bar_loyalty_progress, "value", new_percent, duration)
